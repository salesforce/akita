...
describe('NgEntityService', () => {
  describe('should merge config in order...', () => {
    ...
    describe('', () => {
      ...
      it('default --> global --> decorator', inject(
        [TestServiceWithDecoratorConfig],
        (service: TestServiceWithDecoratorConfig) => {
          const config = service.getConfig();
          expect(config).toEqual({
    ...
    describe('', () => {
      ...
      it('default --> global --> decorator --> inline', inject(
        [TestServiceWithDecoratorAndInlineConfig],
        (service: TestServiceWithDecoratorAndInlineConfig) => {
          const config = service.getConfig();
          expect(config).toEqual({
            baseUrl: 'inline-url',
            httpMethods: {
    ...
    describe('', () => {
      ...
      it('default --> global --> undefined --> inline', inject(
        [TestServiceWithInlineConfig],
        (service: TestServiceWithInlineConfig) => {
          const config = service.getConfig();
          expect(config).toEqual({
            baseUrl: 'inline-url',
            httpMethods: {
    ...
    describe('', () => {
      ...
      it('default --> global --> decorator:baseUrl --> inline:resourceName', inject(
        [TestServiceWithMixedConfig],
        (service: TestServiceWithMixedConfig) => {
          const config = service.getConfig();
          expect(config).toEqual({
            baseUrl: 'decorator-url',
            httpMethods: {
  ...
  describe('resourceName', () => {
    ...
    it('should be the configured resourceName', inject(
      [TestServiceWithDecoratorConfig],
      (service: TestServiceWithDecoratorConfig) => {
  ...
  describe('api', () => {
    ...
    it("should be the configured baseUrl/storeName when resourceName isn't configured", inject(
      [TestService],
      (service: TestService) => {
    ...
    it('should be the configured baseUrl/resourceName', inject(
      [TestServiceWithDecoratorConfig],
      (service: TestServiceWithDecoratorConfig) => {
  ...
  describe('get', () => {
    ...
    it('should dispatch stop loading event when request is complete', fakeAsync(
      inject(
        [TestServiceWithInlineConfig, HttpTestingController, NgEntityServiceLoader],
        (service: TestServiceWithInlineConfig, httpMock: HttpTestingController, loader: NgEntityServiceLoader) => {
          const spy = spyOn(loader, 'dispatch');
          service.get().subscribe();
  ...
  describe('update', () => {
    ...
    it('should transform returned result from request when called with mapResponseFn config', inject(
      ...
      (service: TestServiceWithInlineConfig, httpMock: HttpTestingController) => {
        ...
        const dummyEntity: Partial<TestEntity> = { foo: 'foo', bar: 123 };
        service
          .update(entityId, dummyEntity, { mapResponseFn: x => ({ test: x.foo, test2: x.bar }) })
          .subscribe(result => {
