...
Adds a value to an array if it does'nt exist yet or removes it if already present. Objects are compared by identity by
default. For a fine grained control how values are compared the last argument of `arrayToggle` accepts a compare function.

:::tip
Akita provides two often used comparators for `arrayToggle`: The `byId()` and `byKey(key: string)` compare function.
:::

Toggling objects of an array:
...
Toggling primitive values of an array:

```ts
import { arrayToggle } from '@datorama/akita';

arrayToggle(['a', 'c'], 'b'); // returns ['a', 'b', 'c']
