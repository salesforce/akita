...
describe('FiltersPlugin', () => {
  describe('Manages Filters', () => {
    ...
    describe('SelectALL with Filters ', () => {
      ...
      it('if no filters select all', () => {

         filters.selectAllByFilter().pipe(take(1)).subscribe(result => {
      ...
      it('if apply filter if provided when select all', done1 => {
      ...
      it('if apply 2 filter if provided when select all', done2 => {
        ...
        filters.selectAllByFilter().pipe(take(1)).subscribe(result1 => {
      ...
      it('if apply 2 filter in current order if provided when select all', done3 => {


        filters.setFilter({id: 'filter2', function: (filter) => filter.complete});
        filters.setFilter({id: 'filter1', function: (filter) => filter.id % 2 === 1});

        filters.selectAllByFilter().pipe(take(1)).subscribe(result2 => {
      ...
      it('if apply 2 filter with specified order if provided when select all', done3 => {
    ...
    describe('SelectALL when change  ', () => {
        jest.useFakeTimers();
      ...
      it('if call set filter must send a new value', done3 => {
        jest.setTimeout(2000);
        let count = 0;
        filters.selectAllByFilter().pipe(skip(1), take(1)).subscribe(result2 => {
          expect(Array.isArray(result2)).toBeTruthy();
      ...
      it('if add element must send a new value', done3 => {
    ...
    describe('SelectALL with Sort ', () => {
      ...
      it('if no sort select all in the correct sens ', () => {
      ...
      it('if apply sort if provided all in other sens', done1 => {
      ...
      it('if apply filter and sort', done2 => {
