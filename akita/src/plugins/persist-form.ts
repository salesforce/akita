...
export class PersistForm<E = any, S extends object = any> extends AkitaPlugin<E, S> {
  ...
  constructor(private formGroup: FormGroupLike, private query: Query<S>, private path: string) {
    super();
    this.onInit();
  }
  ...
  onInit() {
    ...
    this.formChanges = this.formGroup.valueChanges.subscribe(value => this.getStore().setState(state => immutableSetProp(state, this.path, value)));
  }
